...
export default function SecretaryCouncilScoringPage() {
  ...
  return <><div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Nhập điểm hội đồng</h1><p className="mt-2 text-slate-600">Thư ký nhập điểm từng thành viên hội đồng và tính điểm tổng hợp.</p></div><Link href="/dashboard" className="rounded-xl border px-4 py-2 text-sm font-semibold">Dashboard</Link></div>{error ? <div className="rounded-2xl bg-red-50 p-4 text-red-700">{error}</div> : null}<div className="grid gap-4 lg:grid-cols-3"><div className="space-y-3 lg:col-span-1">{schedules.map((item) => <button key={item.id} onClick={() => void loadScores(item)} className={`w-full rounded-2xl border bg-white p-4 text-left shadow-sm ${selected?.id === item.id ? 'ring-2 ring-blue-300' : ''}`}><p className="font-bold">{item.student?.user?.fullName ?? item.studentId}</p><p className="text-sm text-slate-500">{item.room} - {new Date(item.startTime).toLocaleString()}</p><div className="mt-2"><StatusBadge value={item.status} /></div></button>)}</div><div className="lg:col-span-2">{selected ? <div className="space-y-4 rounded-3xl bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">{selected.council?.name}</h2><button onClick={calculate} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white">Tính điểm tổng hợp</button></div><CouncilScoreTable scheduleId={selected.id} members={selected.council?.members ?? []} scores={scores} summary={summary} onSubmit={submit} /></div> : <div className="rounded-3xl bg-white p-6">Chưa có lịch bảo vệ.</div>}</div></div></div></>;
}
