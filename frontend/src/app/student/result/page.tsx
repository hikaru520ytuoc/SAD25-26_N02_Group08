...
export default function StudentResultPage() {
  ...
  return <><div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Kết quả bảo vệ của tôi</h1><p className="mt-2 text-slate-600">Sinh viên chỉ xem được kết quả sau khi Khoa công bố.</p></div><Link href="/dashboard" className="rounded-xl border px-4 py-2 text-sm font-semibold">Dashboard</Link></div>{result ? <StudentResultCard result={result} /> : <div className="rounded-3xl bg-white p-6 shadow-sm">{error || 'Chưa công bố kết quả.'}</div>}</div></>;
}
